...
public sealed class CursorIsolationAndMixedInputTests
{
 ...
 [Fact]public void CameraPath_ContainsNoCursorPositionClipOrVisibilityCalls(){var source=Source("src/GameControlMapper/Services/CameraMouseLookService.cs");Assert.DoesNotContain("TrySetPosition",source);Assert.DoesNotContain("TrySetClip",source);Assert.DoesNotContain("TrySetVisible",source);Assert.DoesNotContain("TryGetPosition",source);}
