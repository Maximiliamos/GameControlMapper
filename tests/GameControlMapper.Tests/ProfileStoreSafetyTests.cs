...
public sealed class ProfileStoreSafetyTests
{
    ...
    [Fact] public async Task OneHundredConcurrentSaves_KeepPrimaryAndBackupParseable(){using var f=new F();await f.Store.SaveAsync(f.Profile);await Task.WhenAll(Enumerable.Range(0,100).Select(async i=>{var p=Valid();p.Name="Test";p.Game=i.ToString();await f.Store.SaveAsync(p);}));Assert.NotNull(JsonSerializer.Deserialize<MapperProfile>(File.ReadAllText(f.Path)));Assert.NotNull(JsonSerializer.Deserialize<MapperProfile>(File.ReadAllText(f.Path+".bak")));}
    ...
    [Fact]public async Task ConcurrentCaseInsensitiveNames_CannotOverwriteEachOther(){using var f=new F();var upper=Valid();upper.Name="Race";var lower=Valid();lower.Name="race";var results=await Task.WhenAll(Capture(()=>f.Store.SaveAsync(upper)),Capture(()=>f.Store.SaveAsync(lower)));Assert.Single(results.Where(x=>x is null));Assert.Single(results.OfType<ProfileValidationException>());Assert.Single(Directory.GetFiles(f.Dir,"*.json").Where(x=>!x.EndsWith(".bak",StringComparison.OrdinalIgnoreCase)));}
    [Fact]public async Task ConcurrentUnicodeEquivalentNames_CannotOverwriteEachOther(){using var f=new F();var composed=Valid();composed.Name="caf\u00e9";var decomposed=Valid();decomposed.Name="cafe\u0301";var results=await Task.WhenAll(Capture(()=>f.Store.SaveAsync(composed)),Capture(()=>f.Store.SaveAsync(decomposed)));Assert.Single(results.Where(x=>x is null));Assert.Single(results.OfType<ProfileValidationException>());var saved=await f.Store.LoadAsync("caf\u00e9");Assert.Equal("caf\u00e9",saved.Name);}
    ...
    private static async Task<Exception?> Capture(Func<Task> operation){try{await operation();return null;}catch(Exception exception){return exception;}}
