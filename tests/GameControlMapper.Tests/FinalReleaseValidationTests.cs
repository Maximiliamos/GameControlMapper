...
public sealed class FinalReleaseValidationTests
{
 ...
 [Fact]public void GuidedEvidence_HoldRequiresDurationOrUpdate(){var s=Session();var t=new TouchLifecycleTracker();s.BeginScenario(2,t);var start=DateTimeOffset.Now.AddMilliseconds(10);t.Process(7,10,10,HarnessTouchState.Down,start);t.Process(7,10,10,HarnessTouchState.Up,start.AddMilliseconds(600));Assert.True(s.CanPass(2,t,out _));}
 ...
 [Fact]public void GuidedEvidence_CameraRequiresMeasuredTwoMinutes(){var s=Session();var t=new TouchLifecycleTracker();s.BeginScenario(9,t);var start=DateTimeOffset.Now.AddMilliseconds(10);t.Process(7,10,10,HarnessTouchState.Down,start);t.Process(7,20,10,HarnessTouchState.Move,start.AddMinutes(1));t.Process(7,20,10,HarnessTouchState.Up,start.AddMinutes(2));Assert.True(s.CanPass(9,t,out _));}
 ...
 private static (int ExitCode,string Output) RunValidation(Action<ManualValidationReport> mutate)
 {
  ...
  try
  {
   ...
   var session=Session();Complete(session);var report=session.CreateReport(new TouchLifecycleTracker(),"1.0.0-beta.2","abc123","abc123",app,harness,monitorProvider:new FakeMonitorProvider(SingleMonitor));
   foreach(var scenario in report.Scenarios.Where(x=>GuidedValidationSession.RequiresMachineEvidence(x.Id))){scenario.Evidence=new ScenarioEvidence{StartedAt=DateTimeOffset.Now.AddSeconds(-1),CompletedAt=DateTimeOffset.Now,EventCount=1,AutomaticVerdict=ValidationStatus.Passed,AutomaticReason="test evidence"};scenario.UserVerdict=ValidationStatus.Passed;scenario.FinalVerdict=ValidationStatus.Passed;}
   mutate(report);
