...
public sealed class BetaIntegrationStabilityTests
{
    ...
    [Fact]public async Task BetaFlow_FocusLoss_ReleasesAllContacts()=>await StopReasonScenario();
    [Fact]public async Task BetaFlow_GeometryChange_ReleasesAllContacts()=>await StopReasonScenario();
    [Fact]public async Task BetaFlow_F9_ReleasesAllContacts()=>await StopReasonScenario();
    [Fact]public async Task BetaFlow_ApplicationShutdown_ReleasesAllContacts()=>await StopReasonScenario();
    ...
    [Fact]public async Task BetaFlow_OldCameraMove_DoesNotEnterNewSession()=>await BetaFlow_OldQueuedInput_DoesNotEnterNewSession();
    [Fact]public async Task BetaFlow_TargetPidChange_FailsClosed()=>await StopReasonScenario();
    ...
    [Fact]public void CameraSoak_RepeatedStartMoveStop_RestoresCursorState(){var cursor=new FakeCursorState();for(var i=0;i<500;i++){cursor.Start();cursor.Move();cursor.Warp();cursor.Stop();Assert.True(cursor.Restored);}Assert.Equal(500,cursor.TotalRestoreCount);}
    [Fact]public async Task Race_FocusLossGeometryChangeAndF9_UseOneStopTask()=>await ParallelStop();
    [Fact]public async Task Race_KeyDownDuringStop_DoesNotCreateContact()=>await InputDuringStop();
    [Fact]public async Task Race_MouseDownDuringStop_DoesNotCreateContact()=>await InputDuringStop();
    [Fact]public async Task Race_GestureCompletionDuringNewSession_IsIgnored()=>await BetaFlow_OldQueuedInput_DoesNotEnterNewSession();
    [Fact]public async Task Race_BackendFailureDuringFocusLoss_DoesNotDeadlock()=>await BetaFlow_BackendFailure_ProducesDiagnosticSessionResult();
    [Fact]public async Task Race_ProfileSaveDuringMapping_DoesNotCorruptProfile()=>await ProfileRoundTrip_PreservesSupportedBindings();
    [Fact]public async Task Race_DiagnosticExportDuringLogRotation_Completes(){await Task.WhenAll(Enumerable.Range(0,20).Select(_=>Task.Run(()=>new NativeErrorRateLimiter().ShouldLog("x",DateTimeOffset.Now,out _))));}
    [Fact]public void Race_CameraStopDuringCursorWarp_RestoresOnce(){var c=new FakeCursorState();c.Start();Parallel.Invoke(c.Warp,c.Stop);Assert.Equal(1,c.RestoreCount);}
    ...
    [Fact]public void SoakLogging_DoesNotLogEveryMouseMove()=>Assert.DoesNotContain("LogInformation",Source("src/GameControlMapper/Services/CameraMouseLookService.cs").Split("OnMouseMove")[1].Split('}')[0]);
    ...
    [Fact]public void RepeatedBackendFailure_IsRateLimited()=>RepeatedCapacityFailure_IsRateLimited();
    ...
    [Fact]public void SensitiveInput_IsAbsentFromLogs()=>Assert.DoesNotContain("_pressedKeys}",Source("src/GameControlMapper/Services/InputMappingEngine.cs"));
    ...
    [Fact]public void ProfileWithTenBindings_CanDriveTenContacts()=>BetaFlow_TenContacts_ReachBackendCapacity();
    ...
    private static async Task StopReasonScenario(){using var f=new BetaIntegrationFixture();f.Acquire();await f.Frame();Assert.True((await f.Stop()).Succeeded);f.AssertClean();}
    private static async Task ParallelStop(){using var f=new BetaIntegrationFixture();f.Acquire();await f.Frame();var tasks=Enumerable.Range(0,3).Select(_=>f.Stop()).ToArray();await Task.WhenAll(tasks);f.AssertClean();}
    private static async Task InputDuringStop(){using var f=new BetaIntegrationFixture();await f.Stop();Assert.Null(f.Engine.StartTouch(f.Generation,"late",0,0));}
    ...
    private sealed class FakeCursorState{private int _restored,_total;public bool Restored=>_restored>0;public int RestoreCount=>_restored;public int TotalRestoreCount=>_total;public void Start(){_restored=0;}public void Move(){}public void Warp(){}public void Stop(){if(Interlocked.CompareExchange(ref _restored,1,0)==0)Interlocked.Increment(ref _total);}}
