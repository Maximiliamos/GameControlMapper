...
public sealed class CameraMouseLookServiceTests
{
    ...
    [Fact] public void RelativeNegativeMove_MovesTouchContact(){using var f=new F();f.Camera.OnMouseMove(-10,0,f.Camera.Generation);Assert.True(f.Contact.X<500);}
    ...
    { public FakeCursor Cursor;public FakeTime Time=new();public ContactManager Contacts=MakeContacts();public TouchEngine Engine;public FakeTouchBackend Backend=new();public TouchScheduler Scheduler;public CameraMouseLookService Camera;public TouchContact Contact=>Contacts.ActiveContacts[(int)FixedContacts.Camera];
      ...
      public void Warp()=>Camera.OnMouseMove(0,0,Camera.Generation);public void Real(int dx,int dy){Cursor.Position=new(500+dx,500+dy);Time.Advance(.016);Camera.OnMouseMove(dx,dy,Camera.Generation);}public void Flush()=>Scheduler.SendFrameOnceAsync().GetAwaiter().GetResult();public void Dispose(){Camera.Dispose();Flush();Scheduler.Dispose();}}
