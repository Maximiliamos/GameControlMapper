...
public sealed class CameraMouseLookServiceTests
{
    ...
    [Fact] public void CameraMouseLook_DoesNotRecenterCursor(){using var f=new F();f.Camera.OnMouseMove(10,0,f.Camera.Generation);Assert.Equal(0,f.Cursor.SetCount);}
    [Fact] public void RelativeNegativeMove_MovesTouchContact(){using var f=new F();f.Real(-10,0);Assert.True(f.Contact.X<500);}
    [Fact] public void RealMoveAfterWarp_IsProcessed(){using var f=new F();f.Warp();f.Real(10,0);Assert.True(f.Contact.X>500);}
    [Fact] public void CameraStart_DoesNotReadHideClipOrMoveCursor(){using var f=new F();Assert.True(f.Cursor.Visible);Assert.Equal(0,f.Cursor.SetCount);Assert.Equal(f.Cursor.OriginalClip,f.Cursor.Clip);}
    ...
    [Fact] public void RepeatedStop_NeverTouchesCursor(){using var f=new F();f.Camera.Stop();f.Camera.Stop();Assert.Equal(0,f.Cursor.RestoreVisibleCount);Assert.Equal(0,f.Cursor.SetCount);}
    ...
    [Fact] public void CameraPacketRate_DoesNotChangeSummedMotion(){using var packets=new F(settings:new(){Smooth=0,MaxSpeed=100,DragRadius=500});using var batch=new F(settings:new(){Smooth=0,MaxSpeed=100,DragRadius=500});for(var i=0;i<10;i++)packets.Camera.OnMouseMove(1,0,packets.Camera.Generation);packets.Flush();batch.Real(10,0);Assert.Equal(batch.Contact.X,packets.Contact.X,6);}
    [Fact] public async Task CameraDragRadius_RebasesForUnlimitedRotation(){using var f=new F(settings:new(){Smooth=0,MaxSpeed=100,DragRadius=40});for(var i=0;i<30&&f.Camera.RebaseCount<3;i++){f.Real(30,0);await Task.Delay(5);}Assert.True(f.Camera.RebaseCount>=3);Assert.True(f.Camera.IsActive);Assert.True(f.Backend.RecordedFrames.SelectMany(x=>x.Contacts).Count(x=>x.State==TouchState.Down)>=4);Assert.True(f.Backend.RecordedFrames.SelectMany(x=>x.Contacts).Count(x=>x.State==TouchState.Up)>=3);Assert.All(f.Backend.RecordedFrames.Where(frame=>frame.Contacts.Any(x=>x.State==TouchState.Up)),frame=>Assert.Contains(frame.Contacts,x=>x.State==TouchState.Down));f.Camera.Stop();f.Flush();AssertNoOrphanFrames(f.Backend.RecordedFrames);}
    ...
    [Fact] public void ConcurrentFocusLossAndCtrlUp_SendOneUp(){using var f=new F();Parallel.Invoke(f.Camera.Stop,f.Camera.Stop);f.Flush();Assert.Empty(f.Contacts.ActiveContacts);}
    ...
    [Fact] public void CursorControllerFailure_DoesNotAffectRelativeCamera(){var c=new FakeCursor{FailSet=true};using var f=new F(c);f.Camera.OnMouseMove(5,0);Assert.True(f.Camera.IsActive);Assert.Equal(0,c.SetCount);}
    ...
    { public FakeCursor Cursor;public FakeTime Time=new();public ContactManager Contacts=MakeContacts();public TouchEngine Engine;public FakeTouchBackend Backend=new();public TouchScheduler Scheduler;public CameraMouseLookService Camera;public TouchContact Contact=>Contacts.ActiveContacts[(int)FixedContacts.Camera];
      public F(FakeCursor? cursor=null,bool start=true,CameraSettings? settings=null){Cursor=cursor??new();Engine=new(NullLogger<TouchEngine>.Instance,Contacts);Backend.Initialize();Scheduler=new(NullLogger<TouchScheduler>.Instance,Contacts,Backend,new(),allocator:Engine.ContactAllocator);Camera=new(Engine,NullLogger<CameraMouseLookService>.Instance,Cursor,Time,scheduler:Scheduler);if(start){Camera.Start(settings??new(){Smooth=0,MaxSpeed=100,DragRadius=500},500,500);Scheduler.SendFrameOnceAsync().GetAwaiter().GetResult();}}
      public void Warp()=>Camera.OnMouseMove(0,0,Camera.Generation);public void Real(int dx,int dy){Cursor.Position=new(500+dx,500+dy);Time.Advance(.016);Camera.OnMouseMove(dx,dy,Camera.Generation);Flush();}public void Flush()=>Scheduler.SendFrameOnceAsync().GetAwaiter().GetResult();public void Dispose(){Camera.Dispose();Flush();Scheduler.Dispose();}}
