...
public sealed class CameraMouseLookServiceTests
{
    ...
    [Fact] public async Task CameraDragRadius_RebasesForUnlimitedRotation(){using var f=new F(settings:new(){Smooth=0,MaxSpeed=100,DragRadius=40});for(var i=0;i<30&&f.Camera.RebaseCount<3;i++){f.Real(30,0);await Task.Delay(5);}Assert.True(f.Camera.RebaseCount>=3);Assert.True(f.Camera.IsActive);Assert.True(f.Backend.RecordedFrames.SelectMany(x=>x.Contacts).Count(x=>x.State==TouchState.Down)>=4);Assert.True(f.Backend.RecordedFrames.SelectMany(x=>x.Contacts).Count(x=>x.State==TouchState.Up)>=3);f.Camera.Stop();f.Flush();AssertNoOrphanFrames(f.Backend.RecordedFrames);}
