...
{
    private readonly string _directory;private readonly long _maxBytes;private readonly BlockingCollection<string> _queue=new();private readonly Task _worker;private readonly object _gate=new();private StreamWriter? _writer;private int _disposed;
    ...
    public string DirectoryPath=>_directory;public string CurrentPath=>Path.Combine(_directory,"game-control-mapper.log");
    public void Write(DateTimeOffset timestamp,string level,string category,string message,Exception? exception=null){if(Volatile.Read(ref _disposed)!=0)return;var text=$"{timestamp:O} [{level}] {category}: {Redact(message)}"+(exception is null?"":Environment.NewLine+Redact(exception.ToString()));try{Interlocked.Increment(ref _pending);_queue.Add(text);}catch{Interlocked.Decrement(ref _pending);}}
    ...
    public void WriteCrashReport(string source,Exception exception){try{Directory.CreateDirectory(_directory);File.WriteAllText(Path.Combine(_directory,"crash-report.txt"),$"{DateTimeOffset.Now:O}\nSource: {source}\nType: {exception.GetType().FullName}\n{Redact(exception.ToString())}",new UTF8Encoding(false));}catch{}}
