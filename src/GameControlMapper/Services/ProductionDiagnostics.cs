...
{
    private readonly FileLogSink _logs; private readonly IProfileStore _profiles; private readonly MappingSessionDiagnostics _session; private readonly Models.ApplicationCapabilities _capabilities; private readonly CrashHandlingService? _crashes;
    public DiagnosticExportService(FileLogSink logs,IProfileStore profiles,MappingSessionDiagnostics session,Models.ApplicationCapabilities capabilities,CrashHandlingService? crashes=null){_logs=logs;_profiles=profiles;_session=session;_capabilities=capabilities;_crashes=crashes;}
    public DiagnosticExportService(FileLogSink logs,IProfileStore profiles,MappingSessionDiagnostics session,CrashHandlingService? crashes=null):this(logs,profiles,session,Models.ApplicationCapabilities.Beta,crashes){}
    public async Task ExportAsync(string zipPath,CancellationToken ct=default)
    {
        var temp=Path.Combine(Path.GetTempPath(),"gcm-diag-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(temp);
        try
        {
