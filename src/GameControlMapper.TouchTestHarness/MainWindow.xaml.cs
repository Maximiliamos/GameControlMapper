...
{
    private readonly TouchLifecycleTracker _tracker=new();private readonly GuidedValidationSession _guided=new();private readonly Dictionary<int,FrameworkElement> _markers=[];private readonly string _commit;
    public MainWindow(){InitializeComponent();_commit=Arg("--commit")??"not supplied";ScenarioList.ItemsSource=_guided.Scenarios;Loaded+=(_,_)=>{UpdateWindowInfo();UpdateUi();var export=Arg("--export-report");if(!string.IsNullOrWhiteSpace(export)){var report=_guided.CreateReport(_tracker,Arg("--version")??"1.0.0-rc.1",_commit,Arg("--application-commit")??_commit,Arg("--application-archive")??"",Arg("--harness-archive")??"");GuidedValidationSession.Export(report,export);}};Closing+=(_,_)=>_tracker.RecordActiveContactErrors("window close");}
